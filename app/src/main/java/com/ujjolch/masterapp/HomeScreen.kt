...
//            if user has not selected any units yet then default weight: Kgs
    ...
    if(userdata?.gender == "Male"){
        lastBodyFatPercent = Calculate.BodyFatPercentforMale(age!!,lastBMI)
        secondLastBodyFatPercent = Calculate.BodyFatPercentforMale(age!!,secondLastBMI)
    }
    else if (userdata?.gender == "Female"){
        lastBodyFatPercent = Calculate.BodyFatPercentforFemale(age!!,lastBMI)
                            ...
                            Text(
                                text = if (!NoComparableHist) {
                                    buildAnnotatedString {
                                        append(stringResource(id = R.string.CompareWith))
